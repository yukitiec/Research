...
namespace cv {
    namespace mytracker {
        ...
        double TrackerMOSSE::update(const Mat& image, Rect2d& boundingBox, std::vector<int>& previous_move, bool transport, bool bool_skip, double psrThreshold) {
            ...
            if (transport) {
                center.x = boundingBox.x + boundingBox.width / 2; //left to centerX
                center.y = boundingBox.y + boundingBox.height / 2; //top to centerY
            }
            ...
            if (bool_skip)
            {
                if (previous_psr >= MIN_PSR_SKIP)
                {
                    if (counter_skip >= MAX_SKIP)
                    {
                        std::cout << "max skipping -> delete tracker" << std::endl;
                        previous_psr = 0;
                        return PSR;
                    }
                    ...
                    if (vel_current >= MIN_MOVE_MOSSE || vel_previous >= MIN_MOVE_MOSSE)
                    {
                        ...
                        cosine = (delta_xy.x * (double)previous_move[0] + delta_xy.y * (double)previous_move[1]) / (vel_current * vel_previous); //calculate move successiveness
                    }

                    if (((previous_psr > MIN_PSR_SKIP && PSR <= MIN_PSR_SKIP) && vel_current<1.0 && vel_previous>=3.0) || cosine < Angle_threshold) //appearance and movement change 
                    {
        ...
        double TrackerMOSSE::check_quality(const Mat& image, Rect2d& boundingBox, bool transport) {
            if (H.empty())  // not initialized
                return false;
