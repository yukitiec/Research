...
namespace cv {
    namespace mytracker {
        ...
        private:
            ...
            const double Angle_threshold = -0.05; //change direction abruptly :: 90 degree
            const double delta_psr_threshold = 2.0; //psr delta threshold
            const double MIN_MOVE_MOSSE = 7; //min move
            const int MAX_SKIP = 3; //max number of skip
        public:
            double MIN_PSR_SKIP = threshold_mosse;//threshold_mosse; //for psr skipping condition
            ...
            */
            double update(const Mat& image, Rect2d& boundingBox, std::vector<int>& previous_move, bool transport = true, bool bool_skip = false, double psrThreshold = threshold_mosse);

            double check_quality(const Mat& image, Rect2d& boundingBox, bool transport = true);
            /**
