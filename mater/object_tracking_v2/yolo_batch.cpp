...
void YOLODetect_batch::detect(cv::Mat1b& frame, const int frameIndex, std::vector<std::vector<cv::Rect2d>>& posSaver_left, std::vector<std::vector<cv::Rect2d>>& posSaver_right,
    ...
    std::vector<int>& detectedFrame_left, std::vector<int>& detectedFrame_right, std::vector<int>& detectedFrameClass_left, std::vector<int>& detectedFrameClass_right, int counterIteration,bool& bool_startTracker)
{
    ...
    if (bool_startTracker || !q_tracker2yolo_left.empty() || !q_tracker2yolo_right.empty()) //!q_tracker2yolo_left.empty() || !q_tracker2yolo_right.empty() || 
    {
        bool_startTracker = true;
    ...
    push2Queue_right(existedRoi_right, newRoi_right, existedClass_right, newClass_right, frame, posSaver_right, classSaver_right, frameIndex, detectedFrame_right, detectedFrameClass_right,
        q_yolo2tracker_right);
    q_startTracker.push(true);
}
...
void YOLODetect_batch::getLatestData(std::vector<cv::Rect2d>& bboxes_left, std::vector<cv::Rect2d>& bboxes_right, std::vector<int>& classes_left, std::vector<int>& classes_right)
{
    ...
    {
        //if (queueFrame.empty() || queueFrameIndex.empty()) break;
        if (boolLeft && boolRight)
        {
            std::cout << "############## START YOLO INFERENCE ###############" << std::endl;
            break;
        }
        if (!q_tracker2yolo_left.empty())
        {
            //make instance
            Tracker2yolo new_tracker2yolo_left;
            new_tracker2yolo_left = q_tracker2yolo_left.front();
            q_tracker2yolo_left.pop();
            //classIndex
            if (!new_tracker2yolo_left.classIndex.empty()) {
                classes_left = new_tracker2yolo_left.classIndex;
                boolLeft = true;
            }
            //bbox
            if (!new_tracker2yolo_left.bbox.empty()) bboxes_left = new_tracker2yolo_left.bbox;
            //bool start = queueStartYolo_left.front();s
            //std::cout << " ############## GET LEFT INFO ################# classes_left.size()="<<classes_left.size()<<", bboxes_left.size()="<<bboxes_left.size() << std::endl;
        }
        if (!q_tracker2yolo_right.empty())
        {
            //make instance
            Tracker2yolo new_tracker2yolo_right;
            new_tracker2yolo_right = q_tracker2yolo_right.front();
            q_tracker2yolo_right.pop();
            //classIndex
            if (!new_tracker2yolo_right.classIndex.empty()) {
                classes_right = new_tracker2yolo_right.classIndex;
                boolRight = true;
            }
            //bbox
            if (!new_tracker2yolo_right.bbox.empty()) bboxes_right = new_tracker2yolo_right.bbox;
            //bool start = queueStartYolo_left.front();s
            //std::cout << " ############## GET RIGHT INFO ################# classes_right"<<classes_right.size()<<", bboxes_right="<<bboxes_right.size() << std::endl;
        }
    }
}

void YOLODetect_batch::non_max_suppression2(torch::Tensor& prediction, std::vector<torch::Tensor>& detectedBoxes0, std::vector<torch::Tensor>& detectedBoxes1)
{
...
void YOLODetect_batch::non_max_suppression_unit(torch::Tensor& x, std::vector<torch::Tensor>& detectedBoxes)
{
    /* 1 dimension */
    if (x.dim() == 1)
    {
...
torch::Tensor YOLODetect_batch::xywh2xyxy(torch::Tensor& x)
{
...
void YOLODetect_batch::nms(torch::Tensor& x, std::vector<torch::Tensor>& detectedBoxes)
{
...
float YOLODetect_batch::calculateIoU(const torch::Tensor& box1, const torch::Tensor& box2)
{
...
void YOLODetect_batch::roiSetting(std::vector<torch::Tensor>& detectedBoxes, std::vector<cv::Rect2d>& existedRoi_left, std::vector<int>& existedClass_left, std::vector<cv::Rect2d>& newRoi_left, std::vector<int>& newClass_left,
...
float YOLODetect_batch::calculateIoU_Rect2d(const cv::Rect2d& box1, const cv::Rect2d& box2)
{
...
float YOLODetect_batch::calculateRMSE_Rect2d(const cv::Rect2d& box1, const cv::Rect2d& box2)
{
    float centerX_1 = box1.x + box1.width / 2;
    float centerY_1 = box1.y + box1.height / 2;
    float centerX_2 = box2.x + box2.width / 2;
    float centerY_2 = box2.y + box2.height / 2;
    float dx = std::pow(centerX_1 - centerX_2, 2);
    float dy = std::pow(centerY_1 - centerY_2, 2);
    float rmse = std::pow(dx + dy, 0.5);
    return rmse;
}

void YOLODetect_batch::comparisonTMYolo(std::vector<torch::Tensor>& detectedBoxes, int& candidateIndex, std::vector<cv::Rect2d>& bboxesYolo_left, std::vector<cv::Rect2d>& bboxesYolo_right,
...
void YOLODetect_batch::matchingTracker(int& candidateIndex, std::vector<cv::Rect2d>& bboxesCandidate, std::vector<int>& classIndexesTM, std::vector<cv::Rect2d>& bboxesYolo, std::vector<cv::Rect2d>& existedRoi, std::vector<int>& existedClass)
{
    ...
    float max_iou; //for checking max iou
    float min_rmse;
    ...
    {
        max_iou = IoUThresholdIdentity;      // set max value as threshold for lessening process volume
        min_rmse = Rmse_identity;
            ...
            if (classIndex >= 0)
            {
                ...
                if (candidateIndex == classIndex)
                {
                    ...
                    for (int counterCandidateYolo = 0; counterCandidateYolo < bboxesYolo.size(); counterCandidateYolo++)
                    {
                        ...
                        float rmse = calculateRMSE_Rect2d(bboxesCandidate[counterCandidateTM], bboxesYolo[counterCandidateYolo]);
                        //std::cout << "iou=" << iou << std::endl;
                        if ((classIndex==0 && (iou>=max_iou || rmse<min_rmse)) || (classIndex>0 && rmse <= min_rmse)) // found similar bbox -> circular object -> IoU-based judge, non-circular object -> RMSE-based judge
                        {
...
void YOLODetect_batch::newDetection(int& candidateIndex, std::vector<cv::Rect2d>& bboxesYolo, std::vector<cv::Rect2d>& newRoi, std::vector<int>& newClass)
{
...
void YOLODetect_batch::noYoloDetect(int& candidateIndex, std::vector<cv::Rect2d>& bboxesCandidate, std::vector<int>& classIndexesTM, std::vector<int>& existedClass)
{
    if (candidateIndex == 0)
    {
...
void YOLODetect_batch::push2Queue_left(std::vector<cv::Rect2d>& existedRoi, std::vector<cv::Rect2d>& newRoi,
    ...
    std::queue<Yolo2tracker>& q_yolo2tracker)
{
    ...
    if (!updatedRoi.empty())
    {
        ...
        //classIndex
        newData.classIndex = updatedClassIndexes;
        //bbox
        newData.bbox = updatedRoi;
        //tracker
        newData.tracker = updatedTrackers;
        //template
        newData.templateImg = updatedTemplates;
        //push data
        q_yolo2tracker.push(newData);
    }
...
void YOLODetect_batch::push2Queue_right(std::vector<cv::Rect2d>& existedRoi, std::vector<cv::Rect2d>& newRoi,
    std::vector<int>& existedClass, std::vector<int>& newClass, cv::Mat1b& frame,
    std::vector<std::vector<cv::Rect2d>>& posSaver, std::vector<std::vector<int>>& classSaver,
    const int& frameIndex, std::vector<int>& detectedFrame, std::vector<int>& detectedFrameClass,
    std::queue<Yolo2tracker>& q_yolo2tracker)
...
void YOLODetect_batch::updateData_left(std::vector<cv::Rect2d>& existedRoi, std::vector<cv::Rect2d>& newRoi, std::vector<int>& existedClass, std::vector<int>& newClass,
...
void YOLODetect_batch::updateData_right(std::vector<cv::Rect2d>& existedRoi, std::vector<cv::Rect2d>& newRoi, std::vector<int>& existedClass, std::vector<int>& newClass,
    cv::Mat1b& frame, std::vector<cv::Rect2d>& updatedRoi, std::vector<cv::Ptr<cv::mytracker::TrackerMOSSE>>& updatedTrackers, std::vector<cv::Mat1b>& updatedTemplates,
    std::vector<int>& updatedClassIndexes)
{
    // std::cout << "updateData function" << std::endl;
